...
export default function Checkout() {
  ...
  async function place() {
    ...
    try {
      ...
      // CARD: Prefer PayHere JS SDK popup if available, else fallback to form POST.
      // If backend didn't return payhere (older path), call dedicated create endpoint per guideline.
      if (method === 'CARD' && data.payhere) {
        const payment = { sandbox: !!data.payhere.sandbox, ...(data.payhere.params || {}) }
      ...
      } else if (method === 'CARD' && !data.payhere) {
        // Call dedicated PayHere create endpoint to get SDK params
        const resp = await api.post('/payments/payhere/create', { orderId })
        const sdkParams = resp.data || {}
        if (window.payhere && typeof window.payhere.startPayment === 'function') {
          window.payhere.onCompleted = function () {
            dispatch(clearCart())
            navigate('/orders')
          ...
          window.payhere.onError = function () {}
          try {
            window.payhere.startPayment({ sandbox: !!sdkParams.sandbox, ...sdkParams })
            return
          } catch (e) {
            setError('Payment popup could not be opened. Please try again.')
          }
        } else {
          setError('Payment gateway SDK is not available. Please refresh and try again.')
        }
        return
      }
